...
class FeatureAgent(MahjongGBAgent):
    ...
    def _check_mahjong(self, winTile, isSelfDrawn = False, isAboutKong = False):
        try:
            # env.py:79 pre-increments shownTiles[curTile] before _checkMahjong for self-drawn Hu.
            # FeatureAgent doesn't pre-increment, so compensate: +1 for missing pre-increment + isSelfDrawn.
