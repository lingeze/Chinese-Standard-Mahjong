...
if __name__ == '__main__':
    ...
    if args.model == 'resnet_deep':
        ...
        from model import CNNModel as ModelClass
        model = ModelClass(in_channels=145).to('cuda')
    else:
        from model import ResNetModel as ModelClass
        model = ModelClass(in_channels=145).to('cuda')
    ...
    save_hyperparams(
        ...
        num_epochs=args.epochs, split_ratio=splitRatio,
        loss_fn_name=f'CrossEntropyLoss(smoothing={args.label_smoothing})',
        device='cuda',
    )
